...
namespace Funq.Build
{
    ...
    public class CodeGeneration : ISupportInitialize
    {
        ...
        /// </summary>
        static readonly Func<string, bool> IsCSharpGenericType = type => type.IndexOf('<') != -1 || type.IndexOf('>') != -1;

        /// <summary>
        /// Checks whether the type contains the ` but also the generic type arguments with [[ and ]].
        /// </summary>
        static readonly Func<string, bool> IsFullGenericType = type => Expressions.GenericsArityAndBracket.IsMatch(type);

        /// <summary>
        /// Checks whether the type is a nested type by finding the '+' symbol.
        /// </summary>
        static readonly Func<string, bool> IsNestedType = type => type.IndexOf('+') != -1;

        bool initialized = false;
        List<string> safeImports = new List<string>();
